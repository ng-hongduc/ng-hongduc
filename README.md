# Hi there, I'm Đức Nguyễn (Devin) 👋 

<img src="https://raw.githubusercontent.com/ng-hongduc/ng-hongduc/main/resources/bg-here.png" alt="Hello, Devin here. Nice to meet you.">

<p align="left">
  <img src="https://img.shields.io/badge/Role-AI%20Engineer-0A66C2" alt="Role: AI Engineer">
  <img src="https://img.shields.io/badge/Focus-GenAI%20%2F%20LLMs%20%2F%20MLOps-6F42C1" alt="Focus: GenAI / LLMs/ MLOps">
</p>

---

### 🚀 About Me
I am a passionate **AI Engineer** dedicated to bridging the gap between advanced AI research and production-ready applications. My expertise lies in architecting intelligent systems using **Generative AI / LLMs** and building robust, automated pipelines to deploy them securely at scale.

* 🧠 **What I do:** Design & optimize LLM Applications, RAG systems, and Autonomous AI Agents.
* ⚙️ **My Philosophy:** Building Scalable MLOps Pipelines & Securing LLMs.
* 🤖 **Core Identity:** I build AI, but I remain **HUMAN** — focused on creating technology that solves real-world human problems.

---

### 🛠️ Tech Stack & Toolbox

<table>
  <tr>
    <td align="center" width="96">
      <img src="https://skillicons.dev/icons?i=python" width="48" height="48" alt="Python" />
      <br>Python
    </td>
    <td align="center" width="96">
      <img src="https://skillicons.dev/icons?i=pytorch" width="48" height="48" alt="PyTorch" />
      <br>PyTorch
    </td>
    <td align="center" width="96">
      <img src="https://img.shields.io/badge/LangChain-Framework-1C3C3C" width="70" alt="LangChain: Framework" />
      <br>LangChain
    </td>
    <td align="center" width="96">
      <img src="https://skillicons.dev/icons?i=fastapi" width="48" height="48" alt="FastAPI" />
      <br>FastAPI
    </td>
    <td align="center" width="96">
      <img src="https://skillicons.dev/icons?i=docker" width="48" height="48" alt="Docker" />
      <br>Docker
    </td>
    <td align="center" width="96">
      <img src="https://skillicons.dev/icons?i=githubactions" width="48" height="48" alt="CI/CD" />
      <br>MLOps / CI-CD
    </td>
    <td align="center" width="96">
      <img src="https://skillicons.dev/icons?i=aws" width="48" height="48" alt="AWS" />
      <br>AWS
    </td>
  </tr>
</table>

*   **GenAI / LLMs:** LangGraph | LangChain | RAG | Prompt Engineering | LLM Security (OWASP Top 10 for LLMs)
*   **MLOps & DevOps:** Docker | Kubernetes | MLflow | DVC | GitHub Actions (CI/CD)
*   **Databases:** Vector DBs (Pinecone, ChromaDB, Qdrant) | PostgreSQL | MongoDB

---

### 🔮 Featured Projects
*(Tip: Replace these with your actual repositories!)*

*   **[🎯 Autonomous-AI-Agent-Framework]** - A scalable multi-agent system built with CrewAI and LangChain to automate complex enterprise workflows.
*   **[🛡️ Secure-LLM-Gateway]** - An MLOps pipeline implementing security guardrails against prompt injections and data leaks for production LLMs.
*   **[⚡ Enterprise-RAG-Pipeline]** - A high-performance Retrieval-Augmented Generation pipeline utilizing hybrid search and re-ranking techniques.

---

### 📊 GitHub Stats & Trophy
<p align="left">
  <img src="https://vercel.app" alt="Duc's GitHub Stats" width="400" />
  <img src="https://vercel.app" alt="Top Languages" width="300" />
</p>

---

### 🤝 Connect with Me
<p align="left">
  <a href="https://www.linkedin.com/in/ng-hongduc2002/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2" alt="LinkedIn: Connect" />
  </a>
  <a href="mailto:ng.hongduc05@gmail.com">
    <img src="https://img.shields.io/badge/Email-Contact-green" alt="Email: Contact" />
  </a>
</p>

<p align="center"><i>"Building the future with data, code, and human empathy."</i></p>

[![GitHub Game of Life](https://github4life.herokuapp.com/ethomson.gif?z=6)](https://github4life.herokuapp.com/ethomson)
